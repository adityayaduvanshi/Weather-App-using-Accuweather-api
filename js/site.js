function loadData(system) {
  ...
  $.ajax({
    ...
    .done(function (result) {
      ...
      var placeIDsResourceURL =
        "https://api.flickr.com/services/rest/?method=flickr.places.find&api_key=" +
        flickrApiKey +
        "&query=" +
        location +
      ...
      $.ajax({
        ...
        .done(function (result) {
          ...
          var photosResourceURL =
            "https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=" +
            flickrApiKey +
            "&tags=city&content_type=1&place_id=" +
            placeId +
            "&format=json&nojsoncallback=1";
      ...
      var currentConditionsResourceURL =
      ...
      $.ajax({
        ...
        .done(function (result) {
          ...
          if (system === "metric") {
            ...
            realFeel =
              Math.round(
                result[0].RealFeelTemperature.Metric.Value
              ).toString() + " ℃";
            pressure =
              Math.round(result[0].Pressure.Metric.Value).toString() + " mb";
            windSpeed =
              Math.round(result[0].Wind.Speed.Metric.Value).toString() +
              " km/h";
            beaufort =
              getBeaufort(
                Math.round(result[0].Wind.Speed.Metric.Value),
                system
              ) + " B";
            windDirection =
              result[0].Wind.Direction.Degrees +
              "°" +
              " (" +
              result[0].Wind.Direction.English +
              ")";
            visibility =
              Math.round(result[0].Visibility.Metric.Value).toString() + " km";
          }
          // Imperial
          else {
            ...
            realFeel =
              Math.round(
                result[0].RealFeelTemperature.Imperial.Value
              ).toString() + " °F";
            pressure =
              Math.round(result[0].Pressure.Imperial.Value).toString() +
              " inHg";
            windSpeed =
              Math.round(result[0].Wind.Speed.Imperial.Value).toString() +
              " mph";
            beaufort =
              getBeaufort(
                Math.round(result[0].Wind.Speed.Imperial.Value),
                system
              ) + " B";
            windDirection =
              result[0].Wind.Direction.Degrees +
              ...
              ")";
            visibility =
              Math.round(result[0].Visibility.Imperial.Value).toString() +
              " mi";
          }
      ...
      $.ajax({
        ...
        .done(function (result) {
          ...
          for (var i = 0; i < 12; i++) {
            ...
            var hourlyIcon =
              "icons/conditions/" + result[i].WeatherIcon + ".svg";
            var hourlyTemp =
              Math.round(result[i].Temperature.Value).toString() + "°";
      ...
      $.ajax({
        ...
        .done(function (result) {
          ...
          var moonPhase =
            '<i class="wi ' +
            getMoonPhaseClass(result.DailyForecasts[0].Moon.Age) +
            '"></i>' +
            result.DailyForecasts[0].Moon.Phase.replace(/([A-Z])/g, " $1");
          ...
          for (var i = 0; i < 5; i++) {
            ...
            var icon =
              "icons/conditions/" + result.DailyForecasts[i].Day.Icon + ".svg";
            var tempHigh =
              Math.round(
            ...
            var precipDay =
              result.DailyForecasts[i].Day.PrecipitationProbability + "%";
            var precipNight =
              result.DailyForecasts[i].Night.PrecipitationProbability + "%";
...
const btnOpen = document.querySelector(".btn-newpage");
const newPage = document.querySelector(".newPage");
const btnClose = document.querySelector(".page__close");
btnOpen.addEventListener("click", function () {
  newPage.classList.add("showpage");
});
btnClose.addEventListener("click", function () {
  newPage.classList.remove("showpage");
});
const textSearch = document.querySelector(".type__city");
const curbox = document.querySelector(".curbox");
document.querySelector("form").addEventListener("submit", function () {
  textSearch.classList.add("hidetext");
  curbox.classList.add("toshow");
});
